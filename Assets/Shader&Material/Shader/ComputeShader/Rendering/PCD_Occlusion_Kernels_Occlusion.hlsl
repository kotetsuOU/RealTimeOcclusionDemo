...
void ComputeOcclusion(uint3 id : SV_DispatchThreadID)
{
    ...
    // 【新規性①】Tagに基づく探索スキップ
    // Tag最適化ON: 背景との内積計算はスキップし、対象は仮想メッシュ(1u)のみ
    // Tag最適化OFF: タグを使わず全ピクセルを同列に扱うため、ここではreturnしない
    ...
    // Tag最適化OFF時のみ、背景(2u)も100m奥の仮想座標として計算対象にする
    ...
    for (int searchY = minBound.y; searchY <= maxBound.y; searchY++)
    {
        for (int searchX = minBound.x; searchX <= maxBound.x; searchX++)
        {
            ...
            // 【新規性①】Tagに基づく探索スキップ
            // Tag最適化ON: 近傍は実点群(0u)のみ、背景(2u)は内積計算スキップ
            // Tag最適化OFF: 近傍は全タイプ(0/1/2)を同列に扱い、背景も内積計算対象
    ...
    else
    {
        ...
        // Tag最適化ON時のみ背景(2u)を不透明で維持する
        // Tag最適化OFF(全点同列)時は背景にもalphaを適用し、実点群との重なりを抑える
