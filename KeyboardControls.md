...
| アクション | キー (Key) | 詳細 |
|:---|:---|:---|
| **撮影 (Screenshot)** | `Enter` / `Return` | 「オクルージョンDebugMap」「統合DepthMap」「現在のカメラビュー」を同時保存します（保存先: `Assets/HandTrackingData/OcclusionMaps` / `Assets/HandTrackingData/DepthMaps/Integrated`） |
...
| **Origin Debug Map** | `O` | `Enable Origin Debug Map` を切り替えます。ON のときデバッグ配色ルールで画面上に常時可視化します |
